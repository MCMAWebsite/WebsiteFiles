...
all_conjs_pure_path = file.path(user_website, "pureRDSfiles", "all_conjs_pure")
all_conjs_pure = readRDS(all_conjs_pure_path)

all_conjs_expanded_path = file.path(user_website, "pureRDSfiles", "all_conjs_expanded_pure")
all_conjs_expanded_pure = readRDS(all_conjs_expanded_path)
...
file_list_pure_path = file.path(user_website, "pureRDSfiles", "file_list_pure")
file_list_pure = readRDS(file_list_pure_path)
...
file_list_new_pure = list.files(conj_path)
...
for (filename in file_list_new_pure) {

  filename = file.path(user_website, "conj_data", filename)
  print(paste("Pure cluster Setup Opening", filename))
  ...
  if (str_count(firstLine, ',') == 20) { # if file has trailing commas
    ...
    temp_data = read_csv(filename, skip=1, col_names = colnames[-length(colnames)], col_types = "ccncnnnnnnnnnnnccccc")
  }
...
# Again, how is DataGenerated used in its own definition
...
saveRDS(file_list_pure, file_list_pure_path)
...
#all_conjs_pure = rbind(all_conjs_pure,all_conjs_pure_new)
all_conjs_pure_path = file.path(user_website, "pureRDSfiles", "all_conjs_pure")
saveRDS(all_conjs_pure, all_conjs_pure_path)
...
saveRDS(all_conjs_expanded_pure, file.path(user_website, "pureRDSfiles", "all_conjs_expanded_pure") # save to RDS file
